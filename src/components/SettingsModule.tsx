...
export default function SettingsModule({ users, setUsers, companies, setCompanies, currentUser, tiposReuniao, setTiposReuniao, valoresEmpresa, setValoresEmpresa }: SettingsModuleProps) {
  ...
  const addCompany = async () => {
    ...
    setCompanyError('');
    try {
      // 1. Create Firebase Auth account for sócio
      ...
      if (signUpData.error) {
        if (signUpData.error.message === 'EMAIL_EXISTS') {
          setCompanyError('Este email já possui conta no sistema.');
        } else {
      ...
      // 2. Save company to Firestore
      const newCompanyId = Date.now().toString();
      const newCompany: Empresa = { id: newCompanyId, nome: newCompanyName, segmento: 'Novo', responsavel: newSocioEmail, descricao: '', logo_url: '' };
      await setDoc(doc(db, 'companies', newCompanyId), newCompany);

      // 3. Save sócio user to Firestore
      ...
      setCreatedSocio({ email: newSocioEmail, password: newSocioPassword });
      setNewCompanyName('');
      setNewSocioEmail('');
      setNewSocioPassword('');
    } catch (err: any) {
          ...
          {createdSocio && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-600 mt-2">Envie estas credenciais ao sócio para que ele faça o primeiro login.</p>
